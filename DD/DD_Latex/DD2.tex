		...
		\section{Component interfaces}
			...
			\subsection{Web Server interface and Report interface}
				...
				\subsubsection{REST table of resources}
					\paragraph{}
						...
						\begin{table}[!h]
							...
							\begin{tabular}{L{0.5\textwidth}ccc}
								...
								\midrule
								{\ttfamily /users/registration} & X & - & - \\
								{\ttfamily /users/login} & - & X & - \\
								{\ttfamily /users/authorities/login} & - & X & - \\
								{\ttfamily /reports/default} & X & - & - \\
								{\ttfamily /reports/default/?id=xxx} & - & X & - \\
								{\ttfamily /reports/default/unsafearea} & - & X & - \\
								{\ttfamily /reports/notverified} & - & X & X \\
								{\ttfamily /reports/valid} & - & X & - \\
								{\ttfamily /improvements} & - & X & X \\
								{\ttfamily /statistics/visualize} & - & X & - \\
								{\ttfamily /statistics/download} & - & X & - \\
							\end{tabular}
					...
					\paragraph{}
						...
						\begin{itemize}
							...
							\item {\ttfamily /reports/default/*} represents the resources accessible by the RU, in particular reports and "pseudo report" for the unsafe areas.
				...
				\subsubsection{General request description}
					\paragraph{}
						The data that will be transmitted will be encoded in XML.
						
						To recognize the user who sent a request to the server, the system will rely on tokens. A token is a string that is provided to the user as an answer to the login, it contains information about the user and will always be part of the requests, except the login and sign up.
						...
						\begin{itemize}
							...
							\item Creation time: The token is a "one time only" use. The token lifetime is limited to a session.
						\end{itemize}
						
						In case a request is unsuccessful, a payload will be returned along with a proper HTTP status code. That payload will be an Object containing just one property called "reason", which will contain a brief explanation of the error. 
				\clearpage
				\subsubsection{Detailed requests}
					...
					\paragraph{}
						...
						\paragraph{}
							...
							\begin{table}[!h]
								\begin{tabular}{L{0.2\textwidth}L{0.15\textwidth}L{0.53\textwidth}}
									...
									\midrule
								 	id & String & The email or username of the user \\
						...
						\paragraph{}
							...
							\begin{table}[!h]
								\begin{tabular}{L{0.4\textwidth}L{0.52\textwidth}}
									...
									\textbf{Reason} & \textbf{Description} \\
									\midrule
								  	WrongCredentials & The written credentials does not correspond to any existing user \\
								 	\bottomrule
								\end{tabular}
							\end{table}
							...
							\begin{table}[!h]
								\begin{tabular}{L{0.3\textwidth}L{0.15\textwidth}L{0.43\textwidth}}
									...
									\midrule
								 	...
								 	date & String & The datetime in \newline yyyy-MM-ddThh:mm:ss format \\
								 	highlightVertexes & Object & The coordinates on the picture of where the license plate is located \\
								 	\hspace{2.5mm}$\hookrightarrow$ vertexOne & Object & The coordinates (on the picure) of the top-left vertex \\
								 	\hspace{6.5mm}$\hookrightarrow$ x & Number & The x coordinate of the vertex \\
								 	\hspace{6.5mm}$\hookrightarrow$ y & Number & The y coordinate of the vertex \\
								 	\hspace{2.5mm}$\hookrightarrow$ vertexTwo & Object & The coordinates (on the picture) of the bottom-right vertex \\
								 	\hspace{6.5mm}$\hookrightarrow$ x & Number & The x coordinate of the vertex \\
								 	\hspace{6.5mm}$\hookrightarrow$ y & Number & The y coordinate of the vertex \\
								 	\bottomrule
								\end{tabular}
						...
						\paragraph{}
							...
							\begin{table}[!h]
								\begin{tabular}{L{0.15\textwidth}L{0.15\textwidth}L{0.58\textwidth}}
									...
									\textbf{Field} & \textbf{Type} & \textbf{Description} \\
									\midrule
									id & String & The id that the system has assigned to the sent report. This id will uniquely identify the report\\
								 	\bottomrule
								\end{tabular}
							\end{table}
						...
						\paragraph{}
							...
							\end{table}
						\paragraph{}
							\textcolor{myRed}{\textit{\textbf{Error 404}}} (Not Found)
							\vspace{-2mm}
							\begin{table}[!h]
								\begin{tabular}{L{0.35\textwidth}L{0.57\textwidth}}
									...
									\textbf{Reason} & \textbf{Description} \\
									\midrule
								  	 NoReportError & The requested was not found on the server \\
								 	\bottomrule
								\end{tabular}
							\end{table}
						...
						\paragraph{}
							...
							\begin{table}[!h]
								\begin{tabular}{L{0.3\textwidth}L{0.62\textwidth}}
									...
									\textbf{Reason} & \textbf{Description} \\
									\midrule
								  	 NoReportError & The requested resource was not found on the server \\
								 	\bottomrule
								\end{tabular}
							\end{table}
						...
						\paragraph{}
							...
							\begin{table}[!h]
								\begin{tabular}{L{0.3\textwidth}L{0.15\textwidth}L{0.43\textwidth}}
									...
									\textbf{Field} & \textbf{Type} & \textbf{Description} \\
									\midrule
								 	status & String & The new status of the report \\
								 	\bottomrule
								\end{tabular}
							\end{table}
						\paragraph{}
							\vspace{-2mm}
							\textcolor{myGreen}{\textit{\textbf{Success 204}}} (No content)
						\paragraph{}
							\textcolor{myRed}{\textit{\textbf{Error 403}}} (Forbidden)
							\vspace{-2mm}
							\begin{table}[!h]
						...
						\paragraph{}
							...
							\begin{table}[!h]
								\begin{tabular}{L{0.3\textwidth}L{0.62\textwidth}}
									...
									\textbf{Reason} & \textbf{Description} \\
									\midrule
								  	ReportNotFound & The requested report was not found on the server \\
								 	\bottomrule
								\end{tabular}
							\end{table}
						...
						\paragraph{}
						\vspace{-7.5mm}
						This request gets all the valid reports. Items prefixed with a "?" are optional.
						\paragraph{}
							\vspace{-2mm}
							\textcolor{myBlue}{\textit{\textbf{Parameters}}}
							\vspace{-2mm}
							\begin{table}[!h]
								\begin{tabular}{L{0.3\textwidth}L{0.11\textwidth}L{0.47\textwidth}}
									\toprule
									\textbf{Field} & \textbf{Type} & \textbf{Description} \\
									\midrule
								 	requestType & String & The type of request issued \\
								 	requestField & Object & The field that contains precise information on the request \\
								 	\hspace{2.5mm}$\hookrightarrow$?position & Object & The requested position \\
								 	\hspace{6.5mm}$\hookrightarrow$latitude & String & The requested latitude in DMS \\
								 	\hspace{6.5mm}$\hookrightarrow$longitude & Object & The requested longitude in DMS \\
								 	\hspace{2.5mm}$\hookrightarrow$?radius & Number & The requested radius (m) \\
								 	\hspace{2.5mm}$\hookrightarrow$?violationType & String & The requested violation type \\
								 	\hspace{2.5mm}$\hookrightarrow$?date & String & The requested date (yyyy-MM-dd) \\
								 	\hspace{2.5mm}$\hookrightarrow$?time & String & The requested time slice expressed as its initial hour formatted as hh:00 \\
								 	\bottomrule
								\end{tabular}
							\end{table}
						\paragraph{}
							...
							\vspace{-2mm}
							\begin{table}[!h]
								\begin{tabular}{L{0.3\textwidth}L{0.12\textwidth}L{0.46\textwidth}}
									\toprule
									\textbf{Field} & \textbf{Type} & \textbf{Description} \\
									\midrule
									reports & Object[] & A list of the valid reports of a certain municipality \\
						...
						\paragraph{}
							...
							\begin{table}[!h]
								\begin{tabular}{L{0.3\textwidth}L{0.62\textwidth}}
									...
									\textbf{Reason} & \textbf{Description} \\
									\midrule
								  	NoImprovementError & The requested improvement was not found on the server \\
								 	\bottomrule
								\end{tabular}
							\end{table}
						\clearpage
						...
						\paragraph{}
							...
							\begin{table}[!h]
								\begin{tabular}{L{0.3\textwidth}L{0.15\textwidth}L{0.43\textwidth}}
									...
									\hspace{2.5mm}$\hookrightarrow$points & Object[] & The values of the statistic \\
									\hspace{6.5mm}$\hookrightarrow$x & Number & The value of the first field of the statistic \\
									\hspace{6.5mm}$\hookrightarrow$y & Number & The value of the second field of the statistic \\
								 	\bottomrule
								\end{tabular}
							\end{table}
						\paragraph{}
						...
						\paragraph{}
							...
							\vspace{-2mm}
							\paragraph{}
								The requested file is attached as payload to the response
						\paragraph{}
							\vspace{-2mm}
							\textcolor{myRed}{\textit{\textbf{Error 403}}} (Forbidden)
							\vspace{-2mm}
			...
			\subsection{TS interface and MAS interface}
				\paragraph{}
					The TS and the MAS are external services and their communication interfaces will be implemented trough RESTful APIs. 
					
					URIs, controls on accesses and security will be overlooked for the purpose of this document.
				...
				\subsubsection{Request of data about tickets from the TS}
					...
					\begin{table}[!h]
						\begin{tabular}{L{0.3\textwidth}L{0.15\textwidth}L{0.43\textwidth}}
							...
							\hspace{2.5mm}$\hookrightarrow$date & String & The datetime in \newline yyyy-MM-ddThh:mm:ss format \\								 	\bottomrule
								\end{tabular}
							\end{table}
					\clearpage
					\subsubsection{Forwarding data about valid reports to the TS}
			...
			\subsection{DBMS interface}
				\paragraph{}
					...
					In the database there will be four main different tables: the table of the reports, the table of the users, the table of the authorities and the table of the improvements (see also \ref{fig:classDiagram}).
				
			\subsection{Map Service interface}
				\paragraph{}
					The map service that will be adopted is the one provided by Google, in particular the Geocoding API and the maps SDK will be used.
			...
			\subsection{Web and Application Server interfaces}
				\paragraph{}
					The Web server has one interface, responsible for receiving REST requests and handling them by responding directly, if possible, otherwise by forwarding to the Router through the RouterInterface. 
					
					The Application server has one external interface, the RouterInterface, and has more internal interfaces, one for each of its components, as described below.
				\clearpage
...
database, issued in the municipality specified by the municipalityID, which have still to be verified
				...
				\subsubsection{ManageReportMining}
					...
					\paragraph{}
							\textbf{getUnsafeAreas}: this method is used for fetching from the database all the pseudoreports issued within 300 meters from the given position. A pseudoreport is like a report, but containing only info about violation's type, date and time of issuing. 
							\clearpage
							\subparagraph{}
							\textcolor{myBlue}{\textit{\textbf{Parameters}}}
							\vspace{-2mm}
				...
				\paragraph{}
							...
							\end{itemize}
							The necessary parameters are contained in the "requestField" object. 
							\subparagraph{}
							\vspace{-3mm}
							\textcolor{myBlue}{\textit{\textbf{Parameters}}}
							\vspace{-2mm}
								\begin{table}[!h]
									\begin{tabular}{L{0.2\textwidth}L{0.2\textwidth}L{0.48\textwidth}}
										...
										\midrule
								  		...
								  		requestField & RequestField & The necessary parameters for the request \\
								  		token & Token & The token that identifies the authority in the system \\
								 		\bottomrule
									\end{tabular}
								\end{table}
...
municipalityID & String & The municipality of the requesting local officer \\
				...
				\subsubsection{UserHandler}
					...
					\vspace{-2mm}
							...
							\vspace{-2mm}
								\begin{table}[!h]
									\begin{tabular}{L{0.23\textwidth}L{0.15\textwidth}L{0.5\textwidth}}
										\toprule
										\textbf{Name} & \textbf{Type} & \textbf{Description} \\
										\midrule
				...
				\subsubsection{AuthorityHandler}	
					\paragraph{}
							\textbf{verifyAuthorityCredentials}: this method will be used while logging in an authority, in order to verify the correctness of the inserted credentials
							\subparagraph{}
							\vspace{-3mm}
							\textcolor{myBlue}{\textit{\textbf{Parameters}}}
							\vspace{-2mm}
				...
				\subsubsection{ReportHandler}
					...
					\paragraph{}
							...
							\vspace{-2mm}
								\begin{table}[!h]
									\begin{tabular}{L{0.22\textwidth}L{0.2\textwidth}L{0.46\textwidth}}
										\toprule
										\textbf{Name} & \textbf{Type} & \textbf{Description} \\
										\midrule
				...
				\subsubsection{ImprovementHandler}
					...
					\paragraph{}
							...
							\vspace{-6mm}
								...
								\vspace{-2mm}
									...
									\begin{tabular}{L{0.3\textwidth}L{0.62\textwidth}}
										...
										\textbf{Type} & \textbf{Description} \\
										\midrule
								  		List<Improvement> & The list of improvements that are suggested \\
								 		\bottomrule
									\end{tabular}
								\end{table}
							\subparagraph{}
						...
						\paragraph{}
							...
							\vspace{-2mm}
								\begin{table}[!h]
									\begin{tabular}{L{0.22\textwidth}L{0.15\textwidth}L{0.51\textwidth}}
										\toprule
										\textbf{Name} & \textbf{Type} & \textbf{Description} \\
										\midrule
		...
		\section{Selected architectural styles and patterns}
			\paragraph{}
				To build the system a number of different architectural style choices have been made. This section detail the architectural styles and patterns that were used to build the system:
					\begin{description}
						\item [Client-Server] : The client server style is chosen for the interaction between the user and the server components. This architectural style is based on the request-response pattern, that is the one that mostly fits the way actors interact with the system: they make requests invoking some services provided by the server, and the server provides a response according to the received request.
						\item [Four tier architecture]: The architecture of the whole system has been organized in four tiers: Presentation, Application 1, Application 2 and Data access. In the first tier, the presentation one, there are both the user's app and the authority's web app. The presentation tier will be responsible for almost no computation (only the logic behind the request and the display of the UI will be handled), forwarding all data request to the two following tiers. The first application tier, Application 1, is responsible of receiving the request from all users (RU, ME and LO) and to eventually forward them to the Application 2 tier, which will be the one really in charge of computation. The Application 1 tier will also protect the following layer from malicious attacks limiting the requests that really reach Application 2: in fact Application 1 will also work as a proxy, directly answering the clients request if it already possess all necessary data. The Data access tier will be the one responsible of holding all information; its role is critical since it will allow previous tiers to rely on a stateless implementation.
						\item [RESTful APIs]: REST was adopted to organize the way parties communicate.  The security of the RESTful APIs will be granted by their availability restriction to the HTTPS protocol. REST paradigm has been chosen both because its use is quite common in these cases, and for its ease of understanding and maintenance. The content of the messages will be formatted using XML, this format was preferred because it offers mature tools to validate, parse and build messages. The choice of REST over SOAP was done because the REST paradigm is much more simple, modern, concise and mantainable. This also allows to use modern and more efficient libraries to build the server, which in turn will reduce costs of implementation and increase runtime performance.
					\end{description}

		\section{Other design decisions}
			\subsection{Framework selection}
				\paragraph{}
					Java Enterprise Edition was selected for the implementation of server components, because it can be used to build reliable and scalable application, modeling the components as Enterprise Java Beans. Moreover, Java Persistence APIs can be used for the interaction with the DBMS.
				\paragraph{}
					Due to the fact that system deals with geographical informations and queries, a geographical enabled database is the most natural choice. In particular, the selected DBMS for this system is PostgreSQL with the PostGIS extension. This allows to rely on the maturity and stability of the Postgres ecosystem on the power of the geographical primitives exposed by the PostGIS extension. With respect to competitors, PostgreSQL with PostGIS the most well documented, complete and powerful solution when geographical data and queries are a critical requiment for a system.
					
				\paragraph{}
					For the webserver, the main alternatives available on the market are Apache2 and Nginx. Between them, Nginx was preferred due to its ease of configuration and its better performance with respect to Apache2: the features where Nginx can be seen as the better alternative with respect to its competitor perfectly aligns with the use that has been intended for the first server. These characteristics are a better overall speed using less resources: in fact Nginx, with its event-driven architecture, can handle thousands of connection (requests) within one processing thread, better process on static contexts, and grants better response as a proxy server.
					
				\paragraph{}
					The choice of using GlassFish over Apache Tomcat can be strictly traced back to the need of running a Java EE application on the server, in fact Tomcat is merely an HTTP server and Java servlet container while Glassfish is full-blown Java EE application servers, including an EJB container and all the other features of that stack. 
			\subsection{Security}
				\paragraph{}
					Some security measures have been put in place to guarantee legitimacy of data requests:
						\begin{itemize}
							\item APIs are available only under the HTTPS protocol, which grants integrity and privacy.
							\item Tokens are signed using strong criptographycal primitives (AES256 for encryption and SHA2 for hashing)
							\item All data retrieving APIs require a valid token to be passed in order to work
							\item Tokens have limited lifetime (the one of a session) 
						\end{itemize}
			\subsection{Service providers}
				\paragraph{}
					For the MS, Google Maps has been chosen. This was decided thanks to its reliability and availability of documentation and use cases.